...
import { requestManager } from '~controllers/services/request'
import { proxyStore as store } from 'store/proxyStore'
...
export class IntegrationRoutingV2 extends EventEmitter {
  ...
  public handle = async <T extends BaseIntegrationRequest>(
    ...
    try {
      ...
      if (isStrictMethod) {
        const requests = (await requestManager.getRequests()) as any
        ...
        if (requests?.length > 0) {
          this.bringUpWindow()
          const currentRequest = first(requests)
          if (
